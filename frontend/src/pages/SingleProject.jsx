...
export const SingleProject = () => {
  ...
  const [cloudinaryUploadInProcess, setCloudinaryUploadInProcess] =
    useState(false);
  const [cloudinaryUploadSuccessful, setCloudinaryUploadSuccessful] =
    useState(false);
  ...
  const handleClickScroll = () => {
    ...
    const yPosition =
      contentRef.current.getBoundingClientRect().top + window.scrollY + yOffset;
  ...
  const moveImageRight = (indexToMove) => {
    setInput((prevInput) => {
      ...
      if (indexToMove < images.length - 1) {
        // Check if the image is not already at the last position
        // Swap the current image with the one next to it
        [images[indexToMove], images[indexToMove + 1]] = [
          images[indexToMove + 1],
          images[indexToMove],
        ];
      }
      return {
  ...
  const moveImageLeft = (indexToMove) => {
    setInput((prevInput) => {
      ...
      if (indexToMove > 0) {
        // Check if the image is not already at the first position
      ...
      {loadingProjects ? (
        ...
        currentProject && (
          <>
            ...
            >
              <SlArrowLeft className="cursor-pointer w-8 h-8  absolute z-20 top-[90vh] left-6 laptop:left-20 hover:scale-125" />{" "}
            </NavLink>
        ...
        `}
              >
                ...
                {isEditing && (
                  ...
                  >
                    <FaPen className="w-4 h-4 hidden laptop:flex" />
                  </button>
                )}
              ...
              <div
                className={`w-screen h-screen laptop:w-[48%] laptop:max-w-[48%] absolute inset-0 top-0 left-0 transition-opacity duration-[1500ms] ${
                  contentIsVisible ? "laptop:opacity-0" : "laptop:opacity-100"
                } `}
              >
                {currentProject.images.length > 1 ? (
                  ...
                  <img
                    ...
                    onLoad={() =>
                      analyzeImageColor(currentProject.images[0].url)
                    }
              ...
              >
                <div className="mb-4 col-span-2 tablet:mt-20">
                  ...
                  <div className="flex gap-2">
                    {editingField === "title" ? (
                      ...
                      <h2 className="text-lg font-heading">{input.title}</h2>
                    )}
                    {isEditing && (
                      <button
                        type="button"
                  ...
                  <div className="flex gap-2">
                    ...
                    {editingField === "year" ? (
                      ...
                      <h3 className="text-lg font-heading">{input.year}</h3>
                    )}{" "}
                    {isEditing && (
                      <button
                        type="button"
                  ...
                  {isEditing && editingField === "category" ? (
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex gap-2 font-body items-center fony-body text-main-dark p-2 rounded-xl cursor-pointer hover:drop-shadow w-fit p-4 bg-medium-white mt-4"
                      >
                ...
                <div className="row-span-2 flex flex-col gap-4 laptop:gap-8 laptop:gap-0">
                  <div className="font-body text-justify max-w-full col-span-2 tablet:col-span-1 laptop:hidden flex gap-2">
                    ...
                    {isEditing && (
                      <button
                        ...
                        onKeyDown={handleKeyPress}
                      >
                        <FaPen className="w-4 h-4 laptop:hidden" />
                      </button>
                    )}
                  </div>
                  ...
                  {currentProject.video &&
                    currentProject.video.length > 0 &&
                    currentProject.video[0].url && (
                      <>
                        <iframe
                          ...
                          (editingField === "video" ? (
                            <div className="flex gap-2 items-center text-center mb-8 font-body text-main-dark">
                              <button
                                type="button"
                                onClick={() => setEditingField("null")}
                                className="flex gap-2 font-body items-center p-2 rounded-xl cursor-pointer hover:drop-shadow w-fit p-4 bg-medium-white"
                              >
                                <FaRegCheckCircle className="w-4 h-4 hover:scale-110" />{" "}
                                Done
                              </button>
                              important! link format for embed
                            </div>
                          ) : (
                            <button
                              type="button"
                              onClick={() => setEditingField("video")}
                              className="flex gap-2 font-body items-center fony-body text-main-dark p-2 rounded-xl cursor-pointer hover:drop-shadow w-fit p-4 bg-medium-white mb-8"
                            >
                              <FaPen className="w-4 h-4  hover:scale-110" />{" "}
                              Change video
                            </button>
                          ))}
                      </>
                    )}
                  {(input.images.length >= 2 && input.video.length === 0) ||
                  input.video[0]?.url.length === 0 ||
                  (input.images.length >= 2 &&
                    input.video.length > 0 &&
                    isEditing) ? (
                    <ul className="flex flex-wrap mb-10 gap-4">
                    ...
                    ((input.images.length === 1 && input.video.length === 0) ||
                      input.video[0]?.url.length === 0 ||
                  ...
                  {isEditing && (
                    <div>
                      ...
                      >
                        <FiPlusCircle className="w-4 h-4 hover:scale-110" /> Add
                        image
                        <input {...getInputProps()} />
                      </button>
                      ...
                      {imageInput.length > 0 && (
                        <div className="border-2 border-main-dark border-dotted p-4 rounded-xl">
                            ...
                            {imageInput.map((file, index) => (
                              <div key={index} className="relative">
                                <img
                                  ...
                                  alt="Preview"
                                  onClick={() =>
                                    handleImageClick(file.preview, index)
                                  }
                                ...
                                <div className="mt-2">
                                  <input
                                    ...
                                    value={
                                      imageDetailsInput[index].photographer
                                    }
                                    onChange={(e) =>
                          ...
                          <p className="font-body p-2">
                            For optimal results, set width (horizontal img) or
                            height (vertical img) to 1500px
                          </p>
                          <p className="font-body p-2 text-xl">
                            Confirm to add and dont forget to save
                          </p>
                          <button
                            type="button"
                            onClick={() =>
                          ...
                          >
                            {cloudinaryUploadInProcess ? (
                              "Uploading..."
                            ) : cloudinaryUploadSuccessful ? (
                              "Success! You can now save"
                            ) : (
                              <>
                                Confirm Images
                      ...
                      {isEditing &&
                        (editingField === "video" ? (
                          ...
                          <button
                            ...
                            className="flex gap-2 font-body items-center fony-body text-main-dark p-2 rounded-xl cursor-pointer hover:drop-shadow w-fit p-4 bg-medium-white mb-8"
                          >
                            <FiPlusCircle className="w-4 h-4  hover:scale-110" />{" "}
                            Add video
                          </button>
                        ))}
                    </>
                    ...
                    <div key={creditIndex} className="mb-6">
                      ...
                      {isEditing ? (
                        <div className="flex gap-2 flex-col laptop:flex-row laptop:items-center mb-4">
                          <label className="font-heading text-bold font-bold text-lg text-base mb-1">
                            Role:
                          </label>
                          <input
                            type="text"
                      ...
                      {isEditing && (
                        <div className="flex flex-wrap gap-4 ">
                          {credit.names.map((nameObj, nameIndex) => (
                            <div
                              key={nameIndex}
                              className="flex flex-col laptop:flex-row laptop:items-center gap-2 mb-2"
                            >
                              <label className="font-heading text-bold text-sm mb-1 ">
                                Name:
                              </label>
                              <input
                                type="text"
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  removeNameField(creditIndex, nameIndex)
                                }
                ...
                <div className="bg-main-white  drop-shadow  p-4 fixed bottom-16 tablet:right-[75%] laptop:right-16 laptop:bottom-[80%] z-20 w-fit h-fit rounded-xl flex gap-6 items-center justify-center">
                  {isEditing ? (
                    <>
                      <button
                        type="submit"
                        className=" text-peach cursor-pointer flex items-center gap-2"
                      >
                        <FaRegCheckCircle className="w-4 h-4 hover:scale-110" />{" "}
                        save{" "}
                      </button>
                      <button
                        ...
                        onClick={() => exitEdit()}
                      >
                        <RxCrossCircled className="w-4 h-4 hover:scale-110" />{" "}
                        exit{" "}
                      </button>
                    </>
                  ) : (
                    <FaPen
                      className="w-4 h-4 text-peach cursor-pointer hover:scale-110"
                      onClick={() => setIsEditing(true)}
                    />
                  )}
                  <FaTrashAlt
                    className="w-4 h-4 text-red-700 cursor-pointer hover:scale-110  "
